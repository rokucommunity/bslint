...
export function validateScope(scope: Scope, files: (BrsFile | XmlFile)[], callables: CallableContainerMap) {
	...
	files.forEach((file) => {
		...
		if (deferred) deferredVarLinter(scope, file, callables, deferred, diagnostics);
	});
...
function isBranchedBlock(block: StatementInfo) {
	if (isIfStatement(block.stat)) return true;
	return (
		isBlock(block.stat) &&
...
function createVarLinter(
	...
	fun.parameters.forEach((p) => {
		...
		args.set(name.toLowerCase(), { name, isParam: true });
	});
	...
	function setLocal(name: string, isIterator: boolean) {
		...
		const local = { name, parent, isIterator, metBranches: 1 };
	...
	function findLocal(name: string): VarInfo | undefined {
		...
		for (let i = 0; i < blocks.length; i++) {
			...
			if (local) {
				...
				if (!local.isUnsafe) return local;
				found = local;
			}
		}
		return found;
	...
	function closeBlock(closed: StatementInfo) {
		...
		if (locals && isBranchedBlock(closed)) {
			...
			locals.forEach((local) => {
				if (local.metBranches !== numBranches) local.isUnsafe = true;
				local.metBranches = 1;
			});
		}
		...
		if (!parent.locals) {
			...
			locals.forEach((local, name) => {
				...
				if (isParentIf) {
					...
					if (parentLocal && !parentLocal.isUnsafe) {
